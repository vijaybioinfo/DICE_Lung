...
################ Type arg makers #################
def _make_abf_args(method, obj1, obj2, wds):
    args = {"--d1": obj1.file(wds),
            "--feature1": obj1.feature,
            "--type1": obj1.type,
            "--N1": obj1.sample_size(wds),
            "--cc-ratio1": obj1.get_cc_ratio(wds),
            "--d2": obj2.file(wds),
            "--feature2": obj2.feature,
            "--type2": obj2.type,
            "--N2": obj2.sample_size(wds),
            "--cc-ratio2": obj2.get_cc_ratio(wds),
            "--p1": method.params.get("p1", 1e-4),
            "--p2": method.params.get("p2", 1e-4),
            "--p12": method.params.get("p12", 1e-5)
            }
    return args

################ Functions #################
def make_run_args(method, obj1, obj2, wildcards):
    factories = {"abf": _make_abf_args}

    if not method.name in factories:
        raise Exception(f"{method.name} is not a coloc option...\nPlease use one of {list(factories.keys())}")
    
    return factories[method.name](method, obj1, obj2, wildcards)
...
def initialize(config):
    ...
    run_method = METHOD(config["METHOD"])

    OUT = os.path.join(config['OutDir'], "Coloc")
    return OUT, T1, T2, run_method