...
class Supplementary_Table:
    ...
    def set_annotation(self, annot: str):
        sep = "," if os.path.splitext(annot)[-1]==".csv" else "\t"
        self.annotation = pd.read_csv(annot, sep=sep, usecols=["Geneid", "gene_name"]).rename(columns={"Geneid": "geneid"})
...
class Read_Data:
    ...
    @staticmethod
    def from_metadata(meta: pd.DataFrame, path: str, project_metadata: pd.DataFrame = None):
        files  = []
        ...
        if not project_metadata is None:
            data = data[data.dataset2.isin(project_metadata.sample_name.tolist())]
...
def supplementary_table_wrapper(metadata: pd.DataFrame, path: str, db: str, annot: str, project_metadata: pd.DataFrame = None, best_pph4: bool = True):
    data = Read_Data.from_metadata(metadata, path, project_metadata)
    ...
    supp = supp.sort_values(["Disease", "Level of disease association", "GWAS dataset name", "Predifined immune cell population", "Cell cluster", "eGene Ensmble ID", "COLOC locus"]).reset_index(drop=True)
...
def main(args):
    ...
    project_metadata = pd.read_csv(args.project_metadata) if not args.project_metadata is None else args.project_metadata
...
if __name__ == "__main__":
    ...
    parser.add_argument("--project-metadata", default=None, help="Project metadata file [Optional]")
