...
################################ COLOC ABF METHOD ########################################
##########################################################################################
run.abf <- function(datasets, p1 = 1e-4, p2 = 1e-4, p12 = 1e-5, sensitivity = NULL){
	###### Run Colocalization ########
	results <- lapply(datasets, FUN=function(X){ coloc.abf(dataset1=X$d1, dataset2=X$d2, p1=p1, p2=p2, p12=p12) })
	...
	if (!(sensitivity == FALSE | is.null(sensitivity))){
		file.rds <- paste0(sensitivity, "/table.rds")
		min.p12.file <- paste0(sensitivity, "/min_p12.tsv")

		sensitivity <- paste0(sensitivity, "/sensitivity/")
        create.ouput.folder(sensitivity)
		
		sensitivity.table <- lapply(names(results), FUN=function(name){plot.sensitivity(results[[name]], paste0(sensitivity, name, ".pdf")) })
		names(sensitivity.table) <- names(results)

		min.p12 <- data.frame(
			Feature = names(sensitivity.table),
			min.p12 = unlist(lapply(sensitivity.table, FUN=function(tab){get.minimum.p12(tab)}))
		)

		saveRDS(sensitivity.table, file=file.rds)
		write.output(min.p12, min.p12.file)
	}
...
main.abf <- function(args){
	...
	########### Run analysis #############
	results = run.abf(datasets, p1=args$p1, p2=args$p2, p12=args$p12, sensitivity=args$out)
...
cli <- function(arguments){
	...
	parser$add_argument("--p1", type="double", default=1e-4, help="Prior probability of association for trait 1")
	parser$add_argument("--p2", type="double", default=1e-4, help="Prior probability of association for trait 2")
	parser$add_argument("--p12", type="double", default=1e-5, help="Prior probability of colocalization")
