...
def make_coloc_matrix(data: pd.DataFrame, 
    ...
    if not metadata is None and not metadata.empty:
        ...
        if "order" in metadata.columns:
            order = metadata.sort_values("order").sample_name.tolist()
        else:
            order = metadata.sample_name.tolist()
...
def get_sensitivity_plots(path: str, associations: pd.DataFrame):
    items = zip(associations["dataset1"], associations["dataset2"], associations["Feature1"], associations["Feature2"])
    
    collected = []
    for d1, d2, f1, f2 in items:
        file = f"{path}/{d1}/{d2}/sensitivity/{f1}_{f2}.pdf"

        if not os.path.exists(file):
            raise FileNotFoundError(f"Sensitivity plot not found for {d1} - {d2} - {f1} - {f2}")
