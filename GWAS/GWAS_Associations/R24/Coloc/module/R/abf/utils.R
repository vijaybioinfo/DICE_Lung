...
plot.sensitivity <- function(results, out){
  ...
  if(!is.nan(results$summary["PP.H4.abf"]) && results$summary["PP.H4.abf"] >= 0.5){
    ...
    pdf(out)
		sensitivity.table <- try(
		sensitivity(results, rule = "H4 > 0.5"),
		silent = TRUE
		)
    dev.off()

    if(inherits(sensitivity.table, "try-error")){
      print("Sensitivity calculation failed... returning empty data.frame")
      return(data.frame())
    }
    return(sensitivity.table)

  } else {
    print("Sensitivity plot can not be created... skipping...")
    return(data.frame())
  }
}

get.minimum.p12 <- function(sensitivity.table){
  if (nrow(sensitivity.table) == 0) return(NA)
  return(min(sensitivity.table[sensitivity.table$pass == TRUE, "p12"]))
}
